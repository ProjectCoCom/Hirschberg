/**
 * Summary: React UI component 'Canvasprimaryview'.
 *
 * What it does: Renders the 'Canvasprimaryview' dashboard interface, manages localized state, and handles user actions.
 *
...
export const CanvasPrimaryView = ({
  ...
  // Divider drag handlers — apply width directly to DOM during drag to avoid
  // React re-render feedback loops that cause the panel to oscillate left/right.
  ...
  const handleDividerPointerDown = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      e.preventDefault();
      const panelEl = terminalsPanelRef.current;
  ...
  const handleDividerPointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      ...
      // Apply directly to DOM — no React state update during drag
      const panelEl = terminalsPanelRef.current;
      if (panelEl) {
  ...
  const handleDividerPointerUp = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    // Commit final width to React state once on release
    const panelEl = terminalsPanelRef.current;
    if (panelEl && dividerDragRef.current) {
      const finalWidth = panelEl.clientWidth;
      setTerminalsPanelWidth(finalWidth);
    }
      ...
      {hasPanels && (
        <>
          ...
          <div
            ...
            style={
              !isDraggingDividerRef.current && terminalsPanelWidth != null
                ? { width: `${terminalsPanelWidth}px` }
                : undefined
            }
          >
