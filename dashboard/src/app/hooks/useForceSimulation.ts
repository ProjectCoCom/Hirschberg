/**
 * Summary: Custom React hook 'Useforcesimulation'.
 *
 * What it does: Abstracts state management, data polling, or backend API actions for 'Useforcesimulation' into a reusable hook.
 *
...
export const useForceSimulation = ({
  ...
  useEffect(() => {
    ...
    if (simRef.current) {
      ...
      sim.on("tick", () => {
        tickCount++;
        if (sim.alpha() < ALPHA_MIN || tickCount > 300) {
          sim.stop();
          // Final snapshot on stop
          const final: GraphNode[] = sim.nodes().map((sn) => ({
            ...sn._gn,
            x: sn.x ?? sn._gn.x,
            y: sn.y ?? sn._gn.y,
            vx: sn.vx ?? 0,
        ...
        // Only update React state every 10 ticks to avoid render storms
